...
class SemanticScholarDataset:
    ...
    def get_download_urls_from_diffs(self, start_release_id :str = None, end_release_id = "latest",datasetname = None) -> Dict[str, Any]:
        if self.api_key is None:
            raise ValueError("API key is required to access the Semantic Scholar Dataset API.")
        ...
        logger.info(f"Getting diffs from release {start_release_id} to release {end_release_id}...")
        response = self.__api_request(url)
        return response
    ...
    def download_diffs(self, start_release_id: str, end_release_id: str, datasetname:str = None):
        if datasetname not in self.AVAILABLE_DATASETS:
            raise ValueError(f"Invalid dataset name. Available datasets: {self.AVAILABLE_DATASETS}")
        
        response = self.get_download_urls_from_diffs(start_release_id, end_release_id, datasetname)
        ...
        for diff in diffs:
            ...
            for i, url in enumerate(update_files):
                save_name = f"{datasetname}_{from_release}_{to_release}_update_{i}.json.gz"
                self.__download_file(url, save_name)
            for i, url in enumerate(delete_files):
                save_name = f"{datasetname}_{from_release}_{to_release}_delete_{i}.json.gz"
                self.__download_file(url, save_name)
